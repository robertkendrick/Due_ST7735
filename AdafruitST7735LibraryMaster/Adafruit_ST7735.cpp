...
//bobk
#ifdef __arm__
		uint8_t response = SPI.transfer(c);
#else
		SPDR = c;
		while(!(SPSR & _BV(SPIF)));
#endif
...
#define DELAY 0x80
PROGMEM static uint8_t //prog_uchar
...
void Adafruit_ST7735::commonInit(uint8_t *cmdList) {
  ...
  if(hwSPI) { // Using hardware SPI
    ...
    SPI.begin();	// For Due, in this mode, NPCS control (Slave select) is left to the user (same as AVR)
//bobk
#ifdef __arm__
	// Default speed set to 4Mhz
	SPI.setClockDivider(21);
	// bobk -- dont need this, its extended SPI
	//SPI.setClockDivider(BOARD_SPI_DEFAULT_SS, 21);
#else
